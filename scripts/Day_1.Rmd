...
The overall idea behind piping is to make easy to read a chain of functions. Pipes `%>%` have been introduced in the `magrittr` pachage but are now a core tool of tidyverse
...
```{r}
## Suppose I want to calculate the logarithm of all the numeric colums in the iris dataset ...
...
## across is used to mutete on all these columns
...
iris %>% 
  group_by(Species) %>% 
  summarize(myratio = mean(Sepal.Width), sd = sd(Sepal.Width))
...
* find the top athletes in terms of absolute number of medals on the olympic games history and per year
...
```{r}
## Athlete participating to the large number of events
dat %>% 
  filter(Sport == "Athletics") %>%  ## here I wanted to focus only on people doing athletics
  group_by(ID,Name,Sport) %>%       ## I group by more columns because I want to retain them in the output of summarise
...
`broom` is a package part of the tidyverse environment which allow to reformat the output of a model (actually of many classes of models, tests, etc) making tidier and easy to use
...
The standard way to look to this model is by usin the "summary" function
```{r}
summary(ing_model)
```
...
iris_nest <- iris %>% 
  nest(data = !c(Species))  ## this line says: plaese nest all columns different from `Species` in a new column called data
...
The overall structuyre of the object better clarify the nested nature of the data ...
...
```{r}
## lapply apply a function to a list and return a list
lapply(iris_nest$data, function(e) dim(e))
```

Voila! Fast, efficient ... but not very clear to read

To cleanup all that, in the tidyverse world you can rely on the functions part of the `purrr` package. What I will show here is the basic idea (so the advantage in readability in comparison to base R is not enormous), but `purrr` contains a bunch of functions which can really improve your R experience
...
In the package you will find a plethora of mapers (map, map_2, pmap, walk, map_dbl, ...)
...
but this type of approach can be really handy if you combine it with modeling, and brooming.
...
```{r}
iris_nest %>% 
  mutate(models = map(data, ~lm(Sepal.Length ~ Sepal.Width, data = .x))) %>%     ## here I'm mapping the modeling
  mutate(coefficients = map(models, ~tidy(.x))) %>%                              ## here I'm mapping the tidying
  unnest(coefficients)                                                          ## here I'm unnesting the coeffieints
...
To do that I have first to reshape my data in "long" format. Thsi is done in tidyverse by the `pivot_longer` function
