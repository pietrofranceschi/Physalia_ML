...
mat <- replicate(n = n/k, sample(x = seq(1,k), size = k), simplify = TRUE)
...
for(i in 1:k) {
  ...
  ## train the model 
  fit = glm(PHENOTYPE ~ ., data = training_set, family=binomial(link="logit"))
  
  #############################################
  ## EVALUATE HERE YOUR CLASSIFICATION MODEL ##
  #############################################
  test_set <- dogs[folds==i,]
  
  predictions = predict(fit, newdata = test_set[,-1], type="response")
  predictions <- ifelse(predictions > 0.5, 1, 0)
  
  TER = sum(predictions != test_set$PHENOTYPE)/length(predictions)
  TP = sum(predictions == 1 & test_set$PHENOTYPE == 1)
  FN = sum(predictions == 0 & test_set$PHENOTYPE == 1)
  TN = sum(predictions == 0 & test_set$PHENOTYPE == 0)
  FP = sum(predictions == 1 & test_set$PHENOTYPE == 0)
  FPR = FP/(FP+TN)
  FNR = FN/(FN+TP)
