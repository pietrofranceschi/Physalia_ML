...
We will use the one-class classifier in supervised mode; that is, with labeled data, 
but only for the normal (majority) class.
Then we will predict and calculate metrics based on both classes.
There are a few different ways we can prepare this data.

We filter out all the outliers from the training set and add them to the test set, 
so you can get a better idea of how the model works for outlier detection.
