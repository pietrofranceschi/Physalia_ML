...
See figure below (0: "normal" data points; 1: anomalies).

![](../material/outlier.png)
...
The **minority class** is often **the class of interest** (think of fraudulent bank transactions, positive disease diagnoses, or intruder detection).
Sometimes these applications are framed as a two-class classification problem, but other times they are called **anomaly**, **outlier**, or **novelty** detection.

Imbalanced classification problems are tricky.
Models can achieve high accuracy by classifying everything as the dominant class.
You can somewhat mitigate this problem by choosing models based on other metrics, such as sensitivity.
You can also downsample the data to balance the classes (which throws out a lot of data) or upsample the infrequent class using techniques like SMOTE (Synthetic Minority Oversampling TEchnique) or ROSE (Random Over-Sampling Examples) to create synthetic data points.
...
One-class classifiers are among the most widely used methods in anomaly detection because they don't require extensive labeled data for training.
One-class classification can either be semi-supervised, where only the normal (major) class is used for training, or unsupervised, where the method can handle anomalies in the training class.
The **one-class SVM** is a popular implementation of one-class classifiers.

### The set-up

We use the package `e1071` for SVM functions.
...
We use a dataset on thyroid disease (hyperthyroidism) -the label- and thyroid blood parameters (TSH, T3, TT\$ etc.) [data from [here](https://medium.datadriveninvestor.com/how-to-do-thyroid-detection-using-machine-learning-d577ef24c88a) and [here](https://campus.datacamp.com/courses/introduction-to-anomaly-detection-in-r/comparing-performance?ex=2)]:
...
We see that this dataset is highly imbalanced: 22 cases (hyperthyroidism) out of 1000 examples.
...
We see that we make zero errors in the majority class (easy), but 2 out of 5 mistakes (40%) in the outliers: this is a lot!
...
Now, let’s compare this to the one-class classifier.
We will use the one-class classifier in supervised mode; that is, with labeled data, but only for the normal (majority) class.
Then we will predict and calculate metrics based on both classes.
There are a few different ways we can prepare this data.

We filter out all the outliers from the training set, but not from the test set.
The regular and one-class SVM are tested for predictions on the same test data set.
...
```{r}
## all training observations belong to one class
table(train_normal_class_label)
```

Again, no fine-tuning of hyperparameters (but you should!):

-   `nu`: is a parameter that controls the trade-off between model complexity and training error

In one-class classification `nu` sets an upper bound on the fraction of outliers (margin errors), and a lower bound on the fraction of support vectors.
For example, if we set `nu` to 0.05, at most 5% of the training data are allowed to lie outside the learned decision boundary.
Therefore, higher `nu` values force more support vectors, which translates to a more complex model (low bias, high variance); at the same time, more errors are allowed, which partially counteracts overfitting.
Still, the larger number of support vectors usually dominates, and large `nu` values lead to the risk of overfitting.

```{r}
# fitting one class SVM on training data- no labels needed! 
one_class_svm_model <- svm(train_normal_class_pred, y = train_normal_class_label,
...
Now we make some errors in the "normal" class (false outliers), but we identify correctly all (100%) of the true outliers:

```{r}
print(one_class_confTest)
```

![](../material/training_boundary.png)

![](../material/decision_boundary.png)
