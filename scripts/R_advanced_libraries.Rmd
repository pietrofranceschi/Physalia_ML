...
The overall idea behind piping is to make easy to read (and perform) a chain of functions. Pipes `%>%` have been introduced in the `magrittr` package but are now a core tool of the _tidyverse_
...
```{r}
## Base R
system.time(read.csv("../data/athlete_events.csv"))
```

```{r}
# data.table
system.time(fread("../data/athlete_events.csv"))
```

```{r}
# tidyverse
system.time(read_csv("../data/athlete_events.csv"))
```
...
baseR <- read.csv("../data/athlete_events.csv")
datat <- fread("../data/athlete_events.csv")
tidyv <- read_csv("../data/athlete_events.csv")
```
...
In DT, new columns are created by using the `:=` operator in the second "place holder" of the call. An important point to remember that can create some confusion is that `data.table` modify the object *by reference*, so the new columns will be added to the initial object even without a specific assignment
...
iris_dt[, myratio := Sepal.Length/Sepal.Width]
...
`broom` is a package part of the tidyverse environment which allow to reformat the output of a model (actually of many classes of models, tests, etc) makingm them tidier and easy to use
...
In the previous step we have been using modelling on sample groups ... could I use the same approach to model the separate variables?
...
so I could nest the long DF and switch on my modelling machinery ;-)
...
Like this I have a tibble of model coefficients ... and now I get them out!

```{r}
pippo %>% 
  dplyr::select(!c(mytest,data)) %>% 
  unnest(coeff)
```
