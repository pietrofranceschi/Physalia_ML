...
* Pipes (`%>%`)
* Tabular data
  * `data.frame`, `tibble`, `data.table`,`tidy.tables`
  * data carpentry (`tidyverse`, `data.table`, `tidytable`)
...
library(tidytable)     ## an effective mix of tidyverse style and data.table speed
```
...
```{r}
## read the data
dat <- read_csv("data/555555athlete_events.csv")
```
...
```{r}
## I've found an inconsistency between unnesting of tidyr and unnesting of tidytables!


iris %>% 
  tidyr::nest(data = !c(Species)) %>% 
  dplyr::mutate(models = map(data, ~lm(Sepal.Length ~ Sepal.Width, data = .x))) %>%     ## here I'm mapping the modelling
  dplyr::mutate(coefficients = map(models, ~tidy(.x))) %>%                              ## here I'm mapping the tidying
