...
iris_PCA <- recipe(~., data = iris) %>%      ## no target variable there 
  ...
  step_pca(all_numeric())                    ## we run the PCA by using prcomp unde the hood
...
PCA is a projection so almost always one is interested in understanding how many variance is a projection representing.

This info is typically happily provided by many PCA packages, in Tidymodels we have to follow another way, by tidyng a part of the prepared recipe. In tidymodels we can indeed tydy out a specific step of our recipe
...
komp_recipe %>% 
  ...
  ggplot() + 
  geom_point(aes(x = PC1, y = PC2, col = Gender), size = 2, alpha = 0.7) + 
  geom_text_repel(aes(x = PC1, y = PC2, col = Gender, label = ifelse(PC1>5,as.character(Genotype),''))) + 
  scale_color_brewer(palette = "Set1") + 
  ggtitle("Komp Scoreplot") + 
  theme_bw() + 
  theme(aspect.ratio = 1)
...
komp_recipe %>% 
  juice() %>% 
  ggplot() + 
  geom_point(aes(x = PC1, y = PC2, col = Genotype == "Null"), size = 2) + 
