...
The steps needed to include UMAP projection in a recipe are included in the `embed` package with also include other methods for embedding predictors in new columns. Also *sparse pca* is ioncluded there
...
komp_recipe <- recipe(~., data = komp1) %>% 
  ...
  step_umap(all_predictors(), neighbors = 8) %>% 
  prep()
```
...
```{r}
library(readr)
MTBSL1 <- read_delim("MTBSL1.tsv", delim = "\t", 
