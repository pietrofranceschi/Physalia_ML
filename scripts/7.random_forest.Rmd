...
for (bag_model in bagged_models) {
  ...
  preds = predict(bag_model, dogs[oob[[i]],-1])
  obs = dogs$PHENOTYPE[oob[[i]]]
  oob_results = rbind.data.frame(oob_results, data.frame("rep"=i, "error"=1-sum(preds == obs)/length(preds)))
  i = i+1
}
```
...
for (bag_model in bagged_models) {
  ...
  if (is.null(bagged_result)) {
   ...
   bagged_result=(i*bagged_result+predict(bag_model, x_test))/(i+1)
  }
i = i+1
}

error = sum(10*abs(bagged_result - (as.integer(y_test)-1)))/(length(y_test)*10)
print(error)
```
