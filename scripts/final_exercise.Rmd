...
We now use the metabolomics data on raspberries (genus *Rubus*) for a wrap-up exercise on the use of machine learning algorithms for predictions.
Some code is already there as hint, however you will mostly have to write your own code (and make your own choices)
...
We see that we have **three colors** (`R`, `Y`, `P`), and 15 varieties nested within color.
Color `P` has only one observation, and we can either remove it or assign it to the pigmented samples (`R`).
...
The number of pairwise correlations between variables is ${n \choose k}$ = ${10\,986 \choose 2}$ = $60\,340\,605$ (a lot!).
Let's see how many are highly correlated:
...
In total, `r num_correlated_vars` pairs of variable have linear correlation coefficient $> 0.99$.
We can consider removing one variable for each pair.
