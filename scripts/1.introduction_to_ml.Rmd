...
```{r}
res <- parameters |> group_by(beta0,beta1) |> mutate(cost = loss_function(x=x, y=y, beta0=beta0, beta1=beta1))
beta_min = res[which.min(res$cost),c("beta0","beta1")]
kable(res)
```

```{r}
cat("Estimated coefficents (values of parameters that minimise the cost function:\n")
kable(beta_min)
```
